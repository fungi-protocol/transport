...
pub trait Transport: Send {
    ...
    /// A connector for dialing peers through the transport's shared default
    /// circuit group. Use [`isolated_connector`](Transport::isolated_connector)
    /// to select a distinct circuit-isolation group.
    fn connector(&self) -> Self::Connector;

    /// A connector bound to `isolation`. Channels opened through connectors of
    /// DIFFERENT isolation groups must not share a transport circuit — so their
