...
use fungi_transport::{
    CircuitIsolationId, ConnectError, Connector, ListenParams, OnionAddr, Transport,
};
...
impl Transport for LazyArtiTransport {
    ...
    fn isolated_connector(&self, isolation: &CircuitIsolationId) -> LazyArtiConnector {
        LazyArtiConnector {
            inner: self.inner.clone(),
...
pub struct LazyArtiConnector {
    inner: Arc<LazyInner>,
    /// Circuit-isolation group this connector is bound to; `None` uses the shared default.
    /// The group's isolation token lives on the bootstrapped transport, so
...
mod tests {
    ...
    /// The isolation-id→connector plumbing: the default connector is unbound,
    /// isolated connectors carry their group, and distinct groups stay
    /// distinct. Resolving the id to an isolation token is the
    /// bootstrapped transport's job, pinned by its own tests; no bootstrap
    /// and no network are involved here.
    #[test]
    fn isolated_connector_carries_the_isolation_id() {
        let t = lazy();
        let (s1, s2) = (
            CircuitIsolationId::generate(),
            CircuitIsolationId::generate(),
        );
        assert_eq!(t.connector().isolation, None);
        assert_eq!(t.isolated_connector(&s1).isolation, Some(s1));
        assert_ne!(
            t.isolated_connector(&s1).isolation,
            t.isolated_connector(&s2).isolation
        );
    }
    ...
    fn lazy_transport_is_send() {
        ...
        let c = t.isolated_connector(&CircuitIsolationId::generate());
