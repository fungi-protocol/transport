...
pub struct ArtiConnector {
    ...
    /// Token for this isolation group; `None` shares the client's default
    /// circuits. All connectors in one group hold the SAME token, so their
    /// streams may share a circuit while other groups' cannot.
