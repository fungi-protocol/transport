...
//!
//! Circuit isolation is likewise the daemon's: an isolated connector dials
//! with its group's SOCKS username, and the daemon separates circuits
//! by credential only under `IsolateSOCKSAuth` (its default). A daemon with
//! that turned off would collapse the isolation silently, so an isolated
...
/// dials with a SOCKS credential derived from the isolation id, so the daemon
/// isolates its circuits from other groups'. The default connector carries
/// none.
#[derive(Debug)]
...
impl TorConnector {
    ...
    /// that would silently collapse the groups onto shared circuits.
...
impl Transport for TorTransport {
    ...
    fn isolated_connector(&self, isolation: &fungi_transport::CircuitIsolationId) -> TorConnector {
        // The isolation id's text form becomes the SOCKS username, so the
        // daemon (IsolateSOCKSAuth) gives this group its own circuits.
        TorConnector::with_credential(self.cfg.clone(), isolation.to_string())
    }
...
mod tests {
    ...
    /// isolation credential as usual.
    #[tokio::test]
    async fn isolated_connect_verifies_isolation_once_then_dials() {
        ...
        let isolation = fungi_transport::CircuitIsolationId::generate();
        let connector = TorTransport::new(TorConfig {
            socks_addr,
            control_addr,
            ..TorConfig::default()
        })
        .isolated_connector(&isolation);
        ...
        assert_eq!(users_rx.recv().await.unwrap(), isolation.to_string());
        assert_eq!(users_rx.recv().await.unwrap(), isolation.to_string());
        assert_eq!(
    ...
    #[tokio::test]
    async fn isolated_connect_fails_fast_when_daemon_disables_isolation() {
        let control = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let control_addr = control.local_addr().unwrap();
        tokio::spawn(fake_control_isolation(
            control,
        ...
        .isolated_connector(&fungi_transport::CircuitIsolationId::generate());
        let addr = OnionAddr::new(format!("{:a<56}.onion", "x"), 9735).unwrap();
    ...
    async fn connector_yields_a_framed_channel_through_the_proxy() {
        ...
        let peer_task = tokio::spawn(async move {
            ...
            let mut ch = fungi_transport::framing::FramedChannel::new(
                sock,
                fungi_transport::framing::DEFAULT_MAX_MSG_LEN,
            );
    ...
    async fn listener_publishes_onion_and_accepts_framed_channels() {
        ...
        let dial = tokio::spawn(async move {
            let sock = tokio::net::TcpStream::connect(("127.0.0.1", local))
                .await
                .unwrap();
            let mut ch = fungi_transport::framing::FramedChannel::new(sock, DEFAULT_MAX_MSG_LEN);
    ...
    /// `isolated_connector` derives the credential from the isolation id:
    /// distinct groups get distinct usernames (distinct circuits), the same
    /// group gets the same one, and the default connector carries none.
    #[test]
    fn isolated_connectors_derive_distinct_credentials() {
        ...
        let c1 = transport.isolated_connector(&s1);
        let c2 = transport.isolated_connector(&s2);
        assert_eq!(c1.credential(), Some(s1.to_string().as_str()));
        assert_ne!(c1.credential(), c2.credential());
        assert_eq!(
            transport.isolated_connector(&s1).credential(),
            c1.credential(),
            "same isolation group, same credential"
        );
    }
}
