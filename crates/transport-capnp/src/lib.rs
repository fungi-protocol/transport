...
enum Command {
    ...
    /// `transport.isolatedConnector(isolationId)` — store the new isolated
        ...
        match cmd {
            ...
            Command::IsolatedConnector {
                ...
                tokio::task::spawn_local(async move {
                    ...
                    let mut request = transport.isolated_connector_request();
                    request.get().set_isolation_id(isolation_id.as_str());
                    ...
                    match request.send().promise.await {
                        ...
                        Err(e) if matches!(e.kind, capnp::ErrorKind::Unimplemented) => {
                            let _ = reply.send(Err(ConnectError::Transport(
                                format!("plugin without circuit-isolation support: {e}").into(),
                            )));
                        }
                        Err(e) => {
                            let _ = reply.send(Err(map_connect_error(e)));
    ...
    A: FromStr + Display + Send + Sync + 'static,
{
    ...
    fn isolated_connector(
        ...
        // The isolation id crosses to the plugin as text; the remote
        // `isolatedConnector` binds it there.
        CapnpConnector {
            tx: self.tx.clone(),
            transport_id: self.transport_id,
            connector_id: tokio::sync::OnceCell::new(),
...
pub struct CapnpConnector<A = MemAddr> {
    ...
    max_msg_len: usize,
    /// This connector's circuit-isolation group (text form), or `None` for
...
impl<A> CapnpConnector<A> {
    ...
    /// `isolatedConnector` when this connector has an isolation id.
    async fn ensure_connector(&self) -> Result<u64, ConnectError> {
        self.connector_id
            .get_or_try_init(|| async {
                match &self.isolation_id {
                    None => {
                        let (reply, rx) = oneshot::channel();
                        self.tx
                    ...
                    Some(isolation_id) => {
                        let (reply, rx) = oneshot::channel();
                        self.tx
                            .send(Command::IsolatedConnector {
                                transport_id: self.transport_id,
    ...
    <T::Listener as Listener>::Channel: SplitChannel + 'static,
{
    ...
    fn isolated_connector(
        &mut self,
        params: transport::IsolatedConnectorParams,
        mut results: transport::IsolatedConnectorResults,
    ) -> Promise<(), capnp::Error> {
        let isolation = pry!(pry!(pry!(params.get()).get_isolation_id()).to_str())
            .parse::<fungi_transport::CircuitIsolationId>()
            .map_err(|e| capnp::Error::failed(e.to_string()));
        let connector = self.backend.isolated_connector(&pry!(isolation));
        let client: connector::Client = capnp_rpc::new_client(ConnectorServer {
            connector: Rc::new(connector),
        });
        results.get().set_connector(client);
