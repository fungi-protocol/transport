...
pub(crate) enum Cmd {
    ...
    Dial {
        target: OnionAddr,
        /// Circuit-isolation group to dial under; `None` uses the backend's
        /// shared default connector.
...
pub(crate) fn parse_args(args: Vec<String>) -> Result<Cli, String> {
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            ...
            "--circuit-isolation" => {
                circuit_isolation = Some(
                    it.next()
                        .ok_or("--circuit-isolation needs a pid-seq id")?
                        .parse::<CircuitIsolationId>()
...
pub(crate) async fn run(cli: Cli) -> Result<(), String> {
    ...
    match &cli.cmd {
        ...
        Cmd::Dial {
            ...
            let connector = match circuit_isolation {
...
mod tests {
    ...
    /// `--circuit-isolation <pid-seq>` selects an isolation group; malformed ids
    /// fail the parse rather than silently dialing unbound.
    #[test]
    fn cli_parsing_accepts_and_validates_circuit_isolation() {
        let args = |sess: &str| {
            [
                "fungi-harness",
                "dial",
        ...
        match cli.cmd {
            Cmd::Dial {
                ...
                assert_eq!(circuit_isolation, Some("4242-1".parse().unwrap()));
            }
    ...
    /// Without `--circuit-isolation`, the dial uses the shared default connector.
    #[test]
    fn cli_parsing_defaults_to_no_circuit_isolation() {
        let cli = parse_args(
            [
                "fungi-harness",
                "dial",
        ...
        match cli.cmd {
            Cmd::Dial {
                circuit_isolation, ..
            } => assert_eq!(circuit_isolation, None),
            _ => panic!("expected a dial command"),
        }
    }
